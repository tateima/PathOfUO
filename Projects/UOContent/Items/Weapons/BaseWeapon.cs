...
public abstract partial class BaseWeapon : Item, IWeapon, IFactionItem, ICraftable, ISlayer, IDurability, IPantheonItem, IAosItem
{
    ...
    public string[] SocketArray => string.IsNullOrEmpty(_sockets) ? new string[] {} : _sockets.Split(',');
    ...
    public override void GetProperties(IPropertyList list)
    {
        ...
        {
            list.Add(1114057, $"Alignment: {_alignmentRaw}"); // ~1_val~
        ...
        if (_socketAmount > 0) {
            list.Add(1114057, $"Sockets: {SocketArray.Length.ToString()}/{_socketAmount.ToString()}");
        ...
        if (_shardPower > 0)
        {
            list.Add(1114057, $"Shard Power: {_shardPower.ToString()}");
        }
    ...
    private void Deserialize(IGenericReader reader, int version)
    {
        ...
        if (version == 11)
        {
            ...
            Talent = _talentIndex < BaseTalent.InvalidTalentIndex
                ? TalentConstructor.ConstructFromIndex(_talentIndex)
                : null;
            _warforged = reader.ReadBool();
            _enchanted = reader.ReadBool();
            _socketAmount = reader.ReadInt();
            _sockets = reader.ReadString();
            _haunted = reader.ReadBool();
            _shardPower = reader.ReadInt();
            _frozen = reader.ReadBool();
            _burning = reader.ReadBool();
            _toxic = reader.ReadBool();
            _electrified = reader.ReadBool();
        }
