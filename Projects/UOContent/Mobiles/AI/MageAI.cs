...
public class MageAI : BaseAI
{
    ...
    public virtual bool IsMasterNecromancer => Core.AOS && Mobile.Skills.Necromancy.Value > 90;
    ...
    {
        if (!Mobile.Controlled && !Mobile.Summoned && Mobile.Mana >= 40 && IsMasterNecromancer && !_summonCooldown)
        {
            // Mobile.DebugSay("Attempting to summon something");
    ...
    {
        if (!Mobile.Controlled && !Mobile.Summoned && Mobile.Mana >= 40 && !_summonCooldown && Mobile is OrcishMage
        {
            IsShaman: true
        })
        {
    ...
    {
        Mobile.Mana -= 40;
        Point3D combatantLocation = Mobile.Combatant.Location;
        combatantLocation.X += Utility.RandomMinMax(1, 3);
        combatantLocation.Y += Utility.RandomMinMax(1, 3);
        creature.MoveToWorld(combatantLocation, Mobile.Map);
        creature.SetLevel();
    ...
    public override bool DoActionCombat()
    {
        ...
        if (Blindness.BlindMobile(mobile)) {
            Mobile newTarget = Blindness.GetBlindTarget(mobile, 8);
            if (newTarget != null)
            {
                c = newTarget;
            }
